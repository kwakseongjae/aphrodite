...
pub fn build(variants: &[Variant], project_name: &str) -> ReactPackage {
    ...
    // Vitest + JSDOM smoke (roadmap beta criterion 5: `npm test` exits 0).
    // Excluded from the published tarball by `.npmignore` (src/**/*.test.*).
    files.insert("vitest.config.ts".into(), build_vitest_config());
    files.insert("src/__tests__/smoke.test.tsx".into(), build_smoke_test());
...
fn build_vitest_config() -> String {
    r#"import { defineConfig } from "vitest/config";

// Auto-generated by aphrodite. JSDOM so the smoke test can render
// every primitive into a DOM and assert it mounts non-empty.
...
/// A self-contained smoke test shipped with every emit. Two assertions:
///   1. the barrel re-exports a healthy number of runtime components
///      (catches build / codegen / export regressions across all 70);
///   2. a curated, prop-light subset mounts into JSDOM non-empty
///      (catches runtime render crashes — bad JSX, CSS-var access, etc.).
/// The render subset is deliberately narrow so the test never produces
/// false failures from components that legitimately need props; the full
/// 70-component SSR coverage lives in the Aphrodite SSR harness.
fn build_smoke_test() -> String {
...
describe("aphrodite emit — smoke", () => {
  it("barrel re-exports a healthy number of components", () => {
    const components = Object.values(Lib).filter(isComponent);
  ...
  const cases: Array<[string, () => JSX.Element]> = [
    ...
    ["Alert", () => <Alert>주의</Alert>],
    ["Hint", () => <Hint>도움말</Hint>],
    ["Code", () => <Code>npm i</Code>],
    ["Kbd", () => <Kbd>⌘</Kbd>],
    ["EmptyState", () => <EmptyState title="결과 없음" />],
    ["Stat", () => <Stat label="매출" value="₩1.2M" />],
  ];

  for (const [name, factory] of cases) {
    it(`<${name}/> mounts non-empty`, () => {
...
mod tests {
    ...
    #[test]
    fn emit_ships_vitest_smoke_test() {
        // Roadmap beta criterion 5: every emit must `npm test` green.
        let pkg = build(&fixture(), "x");
        let pkgjson = pkg.files.get("package.json").unwrap();
        assert!(pkgjson.contains("\"test\": \"vitest run\""), "missing test script");
        assert!(pkgjson.contains("\"vitest\""), "missing vitest devDep");
        assert!(pkgjson.contains("@testing-library/react"), "missing testing-library devDep");
        assert!(pkg.files.contains_key("vitest.config.ts"), "missing vitest.config.ts");
        let smoke = pkg
            .files
            .get("src/__tests__/smoke.test.tsx")
            .expect("missing smoke test");
        assert!(smoke.contains("mounts non-empty"));
        assert!(smoke.contains("from \"../index\""));

        // Stories import `@storybook/react` (not a devDep), so they must be
        // excluded from `tsc --noEmit` or `npm run typecheck` fails out of
        // the box (beta criterion 6). Guards that regression.
        let tsconfig = pkg.files.get("tsconfig.json").unwrap();
        assert!(
            tsconfig.contains("*.stories.tsx"),
            "tsconfig must exclude stories from typecheck"
        );
    }
    ...
    fn full_component_count_is_70() {
        ...
        let tsx_count = pkg.files.keys()
            .filter(|k| k.ends_with(".tsx") && !k.ends_with(".stories.tsx") && !k.ends_with(".test.tsx"))
            .count();
