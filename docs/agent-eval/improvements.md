...
| 16 | 🟠 P1 | Frontmatter parser rejects valid YAML shapes from LLM (C1-forced-palette) | 2026-05-13 (pass 3) | LLM returned `colors:` shape that doesn't match Rust `Frontmatter` struct → parse failed. | `ColorTokens.others` now `BTreeMap<String, serde_yaml::Value>` with `other_palettes()` iterator skipping non-map entries; `split_frontmatter` accepts markerless YAML when the first line looks YAML-shaped. | ✅ shipped — C1 re-run honored all three forced hex values at 50/400/700 anchors. |
