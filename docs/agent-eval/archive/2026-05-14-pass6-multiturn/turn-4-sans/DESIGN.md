...
version: "0.2"
description: "Calm editorial presence for a longevity research clinic — cool blue-grey clarity, unhurried precision, generous breathing whitespace, modern grotesque clarity"
colors:
  primary:
    "50": "#eef1f4"
    "100": "#d5dee6"
...
This system serves a longevity research clinic whose editorial content must feel authoritative yet approachable. The design avoids clinical coldness and instead draws from the quiet confidence of a well-edited research journal — the kind of reading experience that invites long attention rather than demanding it.
...
Inter as the display face brings the clarity and systematic precision of a high-quality grotesque sans-serif. Its open apertures, generous x-height, and balanced proportions communicate modern scientific rigor without sterility. The sans-serif display creates a clean architectural frame for content, pairing naturally with Source Serif 4 for body text — the contrast between crisp headings and warm reading text establishes a rhythm that feels both contemporary and grounded.

The spacing scale has been expanded significantly to give the layout room to breathe. Generous whitespace is not an aesthetic luxury here — it is a functional necessity. Longevity research involves complex ideas that deserve unhurried presentation. The top spacing token now sits at 128px, with the scale extending to 192px for major section divisions. Nothing on the page should feel rushed or compressed.
...
Inter operates as the display sans-serif at weight 700, providing firm structural presence for headings without visual noise. Its glyph shapes are engineered for clarity at large sizes — the letterforms carry subtle personality through refined details like slightly squared curves and distinctive diagonal terminals, but the overriding impression is one of systematic precision. For subheadings, consider dropping to weight 500 to create hierarchy within the display family while maintaining the grotesque's clean authority.
...
The page operates on a twelve-column grid with generous gutters of 36px. Content width for long-form editorial should be constrained to roughly 680px — the sweet spot for comfortable single-column reading. Wider layouts can use the full grid for data visualizations, research imagery, or team sections.

Vertical rhythm follows the expanded spacing scale with deliberate generosity. Paragraphs separate with 36px (spacing-6), sections divide with 128px (spacing-16), and major content blocks receive 192px (spacing-24) of breathing room. The top spacing token anchors at 128px, ensuring that page-level sections, hero moments, and major transitions all carry significant whitespace above them. This generous vertical pacing mirrors the unhurried editorial tone.

Navigation should be minimal — a fixed masthead with the clinic name set in Inter and no more than five top-level items. The design trusts the reader to scroll. No sticky call-to-action bars, no aggressive engagement patterns. The content earns attention through quality.
...
Article pages are the core component. They begin with a large display headline in Inter, an author and date line in Source Serif 4 at caption size with generous tracking, and then flow into long-form body text with pull quotes set in Source Serif 4 italic at a larger size and primary-600 color to provide visual punctuation.
...
Call-to-action components use a filled primary-600 background with white text, sized generously with 24px vertical padding to create comfortable touch targets. Secondary actions use a transparent background with a 1px border in neutral-300. Neither should ever feel urgent or aggressive — the tone is invitational.
...
Do use Source Serif 4 italics for emphasis within body text rather than bold weight. The italic provides scholarly warmth that bold lacks. Reserve bold (Source Serif 4 at weight 700) for labels, metadata, and functional elements where clarity supersedes personality.
